...
const Chat = () => {
    ...
    const handleKeyUp = evt => {
        ...
        if (evt.keyCode === 13 && !evt.shiftKey) {
            const chat = { user, message: value, timestamp: +new Date, channel: channel };
        ...
        user && (
            <Fragment>
                ...
                <div className="border-top border-gray w-100 px-4 d-flex align-items-center bg-light" style={{ minHeight: 90 }}>
                    <textarea className="form-control px-3 py-2" onKeyUp={handleKeyUp} placeholder="Enter a chat message" style={{ resize: 'none' }}></textarea>
                </div>
            </Fragment>
        )
    );
